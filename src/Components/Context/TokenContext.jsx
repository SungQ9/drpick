...
export const TokenProvider = ({ children }) => {
  ...
  const [userData, setUserData] = useState(() => {
    try {
      ...
      const storedMemberAuth = localStorage.getItem('memberAuth');
      ...
      return {
        ...
        memberId: storedMemberId ? storedMemberId : null,
        memberName: storedMemberName ? storedMemberName : null,
        memberAuth: storedMemberAuth ? storedMemberAuth : null,
      };
    } catch (error) {
      // 에러 발생 시 초기화
  ...
  const logout = () => {
    ...
    console.log(
      ...
      'memberAuth : ',
      localStorage.getItem('memberAuth'),
    );
  };
  useEffect(() => {
    // 토큰 변경 시 로그인 여부 업데이트
