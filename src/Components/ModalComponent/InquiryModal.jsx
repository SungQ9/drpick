...
const InquiryModal = ({ onClose }) => {
  ...
  const registerInquiry = async () => {
    ...
    const comments = commentsRef.current ? commentsRef.current.value : commentsRef.current
    ...
    try {
      const res = await axios.post("http://localhost:8080/inquiry/registInquiry", formData, config)
